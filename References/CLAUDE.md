...
> This file is the entry point for the coding agent. It defines a **lead (orchestrator) agent** and **7 specialized sub-agents**, the contracts they share, what runs in parallel vs. serial, and a task board. Read [ARCHITECTURE.md](./ARCHITECTURE.md) and [PRD.md](./PRD.md) first — they are the source of truth this file coordinates. (Rename/symlink to `AGENTS.md` if your tool prefers that.)
...
- Verify the [DEMO_SCRIPT.md](./DEMO_SCRIPT.md) beats land on the real UI; tune copy/timing.
...
- [ ] orchestrator — scaffold + types + db + seed
- [ ] data-agent — mock source + gate + webhook
- [ ] forensics-agent — rdap + tavily + policy + investigate
- [ ] ledger-agent — hash chain + governor + receipt
- [ ] frontend-agent — dashboard + stream + shield
- [ ] voice-agent — ElevenLabs browser agent + tools
- [ ] integration-agent — E2E glue + DEMO_MODE fallbacks
- [ ] demo-agent — run-from-clone + demo polish
